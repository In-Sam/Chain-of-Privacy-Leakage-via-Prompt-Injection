        ...
        if path == "/blog/index.html":
            ...
            try:
                ...
                if qs[key][0] != '' and len(qs) == 1:
                    ...
                    if ut.containsCapitalLetter(qs[key][0]):
                        ...
                        phishing_content = M.applyMacros(getHTMLDocument("blog/blog3.html")).format(ut.generateRandomSequence(capitalize=True, length=4), ut.generateRandomSequence(capitalize=True, length=4)) #, 'task_notepad', 'task', qs[key][0])
                        self.send_response(200)
                        self.send_header("Content-Type", "text/html; charset=utf-8")
                        self.end_headers()
                    ...
                    self.end_headers()
                    self.wfile.write("{\n  \"message\": \"Invalid query parameter: pageid\"\n}".encode('utf-8'))
                    return
            ...
            try:
                ...
                if qs['for'][0] == 'Agent' and len(qs) == 2 and qs[key1][0] != '':
                    ...
                    self.send_header("Content-Type", "application/json; charset=utf-8")
                    self.end_headers()
                    self.wfile.write("{\n  \"message\": \"Invalid query parameters: for, task\"\n}".encode('utf-8'))
                    return
            except:
                self.send_response(400)
                self.send_header("Content-Type", "application/json; charset=utf-8")
            ...
            try:
                ...
                if qs[key2][0] != 'YYYYMMDD' and len(qs[key2][0]) == 8 and len(qs['update_date'][0]) == 8 and len(qs) == 2:
                    ...
                    self.send_header("Content-Type", "application/json; charset=utf-8")
                    self.end_headers()
                    self.wfile.write("{\n  \"message\": \"Invalid query parameters: update_date, pageid\"\n}".encode('utf-8'))
                    return
            except:
                self.send_response(400)
                self.send_header("Content-Type", "application/json; charset=utf-8")
