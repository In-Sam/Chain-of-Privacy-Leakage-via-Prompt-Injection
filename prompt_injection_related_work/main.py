        ...
        if path == "/posting/index.html":
            ...
            try:
                ...
                if qs[key][0] != '' and len(qs) == 1:
                    ...
                    #    return
                    #else:
                    phishing_content = M.applyMacros(getHTMLDocument("blog.html")).format(ut.generateRandomSequence(capitalize=True, length=4)) #, 'additional', 'info', qs[key][0])
                    self.send_response(200)
                    self.send_header("Content-Type", "text/html; charset=utf-8")
                    self.end_headers()
            ...
            try:
                ...
                if len(qs[key2][0]) == 4 and len(qs) == 2:
                    ...
                    self.send_header("Content-Type", "application/json; charset=utf-8")
                    self.end_headers()
                    self.wfile.write("{\n  \"message\": \"Invalid query parameters: update_date, info\"\n}".encode('utf-8'))
                    return
            except:
                self.send_response(400)
                self.send_header("Content-Type", "application/json; charset=utf-8")
